...
def compute_advantage_estimates(observations: npt.NDArray,
                                network_controller: 'MultiModelPPO3.NetworkController',
                                discount_cumulative_rewards: npt.NDArray):
...
def create_trajectories_process(
    ...
    while True:
        ...
        observations = []
        rewards = []
        probs: list[list[npt.NDArray]] = [[] for _ in range(args.num_actors+1)]
        scores = []
        ...
        while observation_count < args.num_observations:
            ...
            new_observations = []
            new_rewards = []
            new_probs = [[] for _ in range(args.num_actors+1)]
            prev_action = 0
...
class MultiModelPPO3:
    '''Model implementing the PPO training algorithm'''
    INFO = '''
    MultiModelPPO3 with cyclic training,
    uses another network to act as an "intelligent" switch between agent actions.
    Also passes the previous actions as inputs to the agents
    '''
    ...
    class NetworkController:
        ...
        def __init__(self,
            ...
            # if load:
            #     return
            # input dims should include the last output_dims
            ...
            self.switch_optimiser = Adam(lr=self.LEARNING_RATE)

            self.last_output = np.zeros(output_dims * output_dims)

        @classmethod
        def load_saved_model(cls, load_location) -> 'MultiModelPPO3.NetworkController':
            ...
            network_controller.last_output = np.zeros(
                network_controller.output_dims * network_controller.output_dims)
            ...
            network_controller.switch_optimiser = Adam(
                lr=network_controller.LEARNING_RATE)
        ...
        def get_prob_action(self, observation: npt.NDArray) -> tuple[list[npt.NDArray], int]:
            ...
            if action >= 4:
                action = 3
            return probs + [prob.numpy()[0]], action
    ...
    class Trainer:
        ...
        def __init__(self,
            ...
            if not 'LOAD' in os.environ or os.environ['LOAD'] != 'true':
                ...
                self.network_controller = MultiModelPPO3.NetworkController.load_saved_model(
                    self.save_location)
                # reset time steps
        ...
        def create_trajectories_parallel(self) -> Trajectories:
            ...
            trajectories = Trajectories(
                ...
                [
                    ...
                    for i in range(self.configs.NUM_AGENTS+1)
                ],
        ...
        def update_policy(self, trajectories: Trajectories):
            ...
            try:
                ...
                index = int(
                    mod_games_played / (self.configs.GAMES_TO_TRAIN_OVER / (self.configs.NUM_AGENTS + 1)))
                actor = self.network_controller.actors[index] \
                    if index < self.configs.NUM_AGENTS else self.network_controller.switch
                with tf.GradientTape() as actor_tape, tf.GradientTape() as critic_tape:
                    ...
                    sub_probabilities: list[ttf.Tensor1] = [
                        actor(np.concatenate(
                            ...
                            trajectories.probabilities[self.configs.NUM_AGENTS]), 2),

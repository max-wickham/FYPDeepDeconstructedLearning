...
def compute_advantage_estimates(observations: npt.NDArray,
    ...
    val_sum = np.where(np.abs(val_sum) < 0, val_sum + 0.001 * np.sign(val_sum), val_sum)
    ...
    # general advantage and then also the advantage for each
    advantages = np.hstack([advantages] * (num_actions + 1))
    votes = np.array(votes)
    votes: npt.NDArray = np.reshape(votes, (votes.shape[0], votes.shape[1] * votes.shape[2]))
    # votes are action1agent1 action1agent2 action1agent3 action2agent1 action2agent2 action2agent3
    # advantages is
    # agent1 agent2 agent3
    # action1agent1 action1agent2 action1agent3
    # action2agent1 action2agent2 action2agent3
    advantages[:, num_agents:] -= np.where(votes < 0.0, 0.0, votes)
    # normalise the advantages along within each column
